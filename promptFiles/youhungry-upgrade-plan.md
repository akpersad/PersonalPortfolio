...
|---|---|---|---|---|
| Technical depth | 25% | 7 | 9 | Good decision engine; nothing *hard* shipped yet (offline started but unfinished, no real-time) |
...
- `README.md:34` still says: *"📸 Note: Create a demo GIF showing the core flow"* and `public/screenshots/DEMO_PLACEHOLDER.md` exists (three real mobile screenshots do exist alongside it).
...
- Note: the Playwright workflow installs with `--legacy-peer-deps` — investigate *why* (which peer conflict) and fix the underlying conflict if possible rather than propagating the flag into the new workflow.
